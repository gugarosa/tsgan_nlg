"""Text-Similarity Generative Adversarial Network with Cross-Entropy Loss-based Discriminator.
"""
...
from core.discriminators import EntropyDiscriminator

logger = l.get_logger(__name__)


class TSGANEntropy(Adversarial):
    """A TSGANEntropy class is the one in charge of Text-Similarity Generative Adversarial Networks
    implementation with Cross-Entropy loss-based discriminator.
    ...
    def __init__(self, encoder=None, vocab_size=1, embedding_size=32, hidden_size=64,
        ...
        super(TSGANEntropy, self).__init__(D, G, name='TSGANEntropy')
        ...
        # Number of pairs to feed the discriminator
    ...
    @property
    def n_pairs(self):
        """int: Number of pairs to feed the discriminator.
    ...
    def create_balanced_pairs(self, data, labels):
        """Creates balanced pairs from data and labels.

        Args:
            data (tf.tensor): Tensor of samples.
        ...
        x1 = tf.convert_to_tensor(x1_p[:n_pairs] + x1_n[:n_pairs])
        x2 = tf.convert_to_tensor(x2_p[:n_pairs] + x2_n[:n_pairs])
        y = tf.convert_to_tensor(y_p[:n_pairs] + y_n[:n_pairs])
