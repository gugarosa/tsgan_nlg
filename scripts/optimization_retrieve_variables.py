...
def get_arguments():
    ...
    parser = argparse.ArgumentParser(usage='Retrieves the best variables from sets of .pkl optimization history files.')
...
if __name__ == '__main__':
    ...
    # Gathers best agent's variables
    # Note that we will perform this for every set of input history files
    best_agent_vars_per_input = [[h.get_convergence('best_agent')[0][:,-1] for h in history]
                                 for history in histories]

    # Peforms a mean calculation over their variables
    mean_best_agent_vars_per_input = [np.mean(var, axis=0) for var in best_agent_vars_per_input]

    # Displays the best variables
    for mean_best_agent_vars in mean_best_agent_vars_per_input:
        print(f'g_lr: {mean_best_agent_vars[0]} | d_lr: {mean_best_agent_vars[1]} | ' +
              f'embedding_size: {int(mean_best_agent_vars[2])} | hidden_size: {int(mean_best_agent_vars[3])} | ' +
              f'tau: {int(mean_best_agent_vars[4])}\n')
